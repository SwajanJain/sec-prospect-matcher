    ...
    async loadProspects(filePath) {
        return new Promise((resolve, reject) => {
            ...
            fs.createReadStream(filePath)
                ...
                .on('data', (row) => {
                    const prospectId = row.prospect_id || row['prospect_id'] || row['Prospect ID'] || row.id;
                    const prospectName = row.prospect_name || row['prospect_name'] || row['Prospect Name'] || row.name;
                    const companyName = row.company_name || row['company_name'] || row['Company Name'] || row.company;
